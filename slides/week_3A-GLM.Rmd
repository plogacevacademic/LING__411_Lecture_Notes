...
```{r, fig.height=2, fig.width=2, fig.align='right'}
inputPanel(
  sliderInput("intercept3", "Intercept", min = -100, max = 100, value = 0),
  sliderInput("slope3", "Slope", min = -50, max = 50, value = 1)
)
```
...
renderPlot({
  ...
  p2 <- errors %>% ggplot(aes(intercept, slope, color = sum_sq)) + geom_point(alpha = 0.25) + theme_bw()
