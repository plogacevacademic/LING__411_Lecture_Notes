...
renderPlot({
  ...
  errors <<- rbind(errors, data.frame(intercept = input$intercept3, slope = input$slope3, avg_err = mean(abs(cars$residuals)) )) %>% unique()
  
  p2 <- errors %>% ggplot(aes(intercept, slope, color = avg_err)) + geom_point(size=5) + theme_bw()
